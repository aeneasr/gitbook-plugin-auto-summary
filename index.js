...
function groupByDir(files) {
  const fileTree = {};
  files.forEach((file) => {
    // glob always returns '/' path separator?
    lodash.set(fileTree, file.split('/'), file)
  });
  return fileTree
}
...
function getSummaryByFileTree(obj, readmeFilename) {
  ...
  lodash.forEach(obj, (child, sub) => {
    if (lodash.isObject(child)) {
      ...
      } else if (sub.toLowerCase().substr(sub.length - 'readme.md'.length, 'readme.md'.length) === 'readme.md') {
        summary.children.unshift({
          file: child,
